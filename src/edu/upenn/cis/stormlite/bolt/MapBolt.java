...
public class MapBolt implements IRichBolt {
	...
	public void prepare(Map<String, String> stormConf, TopologyContext context, OutputCollector collector) {
		...
		if (!stormConf.containsKey("mapper"))
			throw new RuntimeException("Mapper class is not specified as a config option");
		else {
			String mapperClass = stormConf.get("mapper");
		...
		int spout = Integer.valueOf(stormConf.get("spoutExecutors"));
		int mapBolts = Integer.valueOf(stormConf.get("mapExecutors"));
		String[] workers = WorkerHelper.getWorkers(stormConf);
		neededVotesToComplete = spout + mapBolts * (workers.length - 1);
		System.out.println("MapBolt " + getExecutorId() + " : " + neededVotesToComplete);
	}
	...
	public synchronized void execute(Tuple input) {
		System.out.println(getExecutorId().substring(getExecutorId().length() - 4, getExecutorId().length() - 1) + " in MapBolt, still needs" + neededVotesToComplete + " votes to collect");
		if (!input.isEndOfStream()) {
			String key = input.getStringByField("key");
			String value = input.getStringByField("value");
			System.out.println("MapBolt " + getExecutorId() + " received " + key + " / " + value);
			...
			if (neededVotesToComplete == 0) {
				System.out.println("MapBolt " + getExecutorId().substring(getExecutorId().length() - 4, getExecutorId().length() - 1) + " : " + "sending EOS");
