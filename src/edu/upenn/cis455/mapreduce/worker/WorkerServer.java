...
public class WorkerServer {
	...
	public WorkerServer(int myPort) throws MalformedURLException {
		...
		Spark.post(new Route("/definejob") {
			...
			public Object handle(Request arg0, Response arg1) {
				...
				WorkerJob workerJob;
				System.out.println("Reaching worker server");
				try {
					...
					System.out.println("I am in WorkerServer definejob exception");
					e.printStackTrace();
	...
	public static void createWorker(Map<String, String> config) {
		...
		if (!config.containsKey("workerIndex"))
			...
			if (addresses.length == 0) {
				System.out.println("No workers available now!");
				throw new RuntimeException("No workers available now!");
			}
			...
			try {
				...
				// WorkerStatusUpdater updater = new
				// WorkerStatusUpdater(config.get("master"),
	...
	public static void shutdown() {
		...
		cluster.shutdown();
		System.out.println("Workerserver shutting down");
		System.exit(0);
	}
}
